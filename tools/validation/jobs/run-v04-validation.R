...
if (identical(run_state$action, "reuse")) {
  ...
  ended_at <- Sys.time()
  validation_record_reuse(paths, run_state, started_at, ended_at, "Compatible V04 output reused.")
...
ended_at <- Sys.time()
validation_record_manifest(paths, run_id, experiment_id, mode, status, started_at, ended_at, seed_root, fixtures, script_path, result_path, summary_path, "V04 Engine A2 solver convergence and post-solve residual evidence.", parameters = parameters)
print(summary)

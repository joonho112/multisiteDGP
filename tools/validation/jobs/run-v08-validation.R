...
if (isTRUE(resume) && !isTRUE(overwrite) && validation_existing_run_complete(result_path, summary_path)) {
  ...
  ended_at <- Sys.time()
  validation_record_manifest(paths, run_id, experiment_id, mode, status, started_at, ended_at, seed_root, fixtures * reruns, script_path, result_path, summary_path, "Existing V08 output reused.")
...
ended_at <- Sys.time()
validation_record_manifest(paths, run_id, experiment_id, mode, status, started_at, ended_at, seed_root, nrow(results), script_path, result_path, summary_path, "V08 local Engine A2 reproducibility evidence; cross-OS matrix not run locally.")
print(summary)

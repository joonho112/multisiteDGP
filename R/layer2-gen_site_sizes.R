...
#'
#' Above `cv` about 1.5 the draw is heavy-tailed and the solve is delicate, so
#' A2 warns and proceeds. At very small `cv`, A2 evaluates truncated moments
#' with an incomplete-Gamma recurrence to avoid subtracting two nearly equal
#' raw moments. The requested mean must always pass the user's `tol`. The SD
#' check may use a separately estimated roundoff bound, capped at relative
#' `1e-3`; if the SD cannot be verified inside that cap, A2 refuses the design
#' rather than turning numerical uncertainty into permission to miss the
#' target.
#'
